...
cmake "$PROJECT_DIR" -GNinja -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_PREFIX_PATH=/opt/qt/6.3.2/android_armv7 \
    -DCMAKE_TOOLCHAIN_FILE=/opt/qt/6.3.2/android_armv7/lib/cmake/Qt6/qt.toolchain.cmake \
    -DANDROID_SDK=/opt/android-sdk \
    -DANDROID_NDK=/opt/android-sdk/ndk/25.1.8937393 \
    -DANDROID_PLATFORM=android-33 \
    -DANDROID_ABI=armeabi-v7a \
    -DQT_ANDROID_GENERATE_APK=ON || {
    echo "CMake failed for ARMv7"
    exit 1
}
ninja || {
    echo "Ninja failed for ARMv7"
    exit 1
}
echo "Signing ARMv7 APK..."
/opt/android-sdk/build-tools/33.0.0/apksigner sign \
    --ks /src/android/my-release-key.jks \
    --ks-pass pass:password \
    --out android-build/build/outputs/apk/debug/android-build-signed.apk \
    android-build/build/outputs/apk/debug/android-build-debug.apk || {
    echo "Signing failed for ARMv7"
    exit 1
}
...
cmake "$PROJECT_DIR" -GNinja -DCMAKE_BUILD_TYPE=Release \
    -DCMAKE_PREFIX_PATH=/opt/qt/6.3.2/android_arm64_v8a \
    -DCMAKE_TOOLCHAIN_FILE=/opt/qt/6.3.2/android_arm64_v8a/lib/cmake/Qt6/qt.toolchain.cmake \
    -DANDROID_SDK=/opt/android-sdk \
    -DANDROID_NDK=/opt/android-sdk/ndk/25.1.8937393 \
    -DANDROID_PLATFORM=android-33 \
    -DANDROID_ABI=arm64-v8a \
    -DQT_ANDROID_GENERATE_APK=ON || {
    echo "CMake failed for ARM64"
    exit 1
}
ninja || {
    echo "Ninja failed for ARM64"
    exit 1
}
